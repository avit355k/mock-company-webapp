...
public class SearchController {
    ...
    public Collection<ProductItem> search(@RequestParam("query") String query) {
        ...
        for (ProductItem item : allItems) {
            ...
            if((checkItemPresent(query, item.getName(),item.getDescription())) ||
                    (checkItemPresent(query, item.getName().toLowerCase(), item.getDescription().toLowerCase()))) itemList.add(item);
        }
        return itemList;
    }

    private Boolean checkItemPresent(String query, String itemName, String itemDesc) {
        boolean nameMatches = query.equals(itemName);
        boolean descMatches = query.equals(itemDesc);

        if (nameMatches || descMatches) return true;

        String[] itemNames = itemName.split(" ");
        for (String name : itemNames) {
            if (name.equals(query) || itemDesc.contains(query)) return true;
        }
        return false;
    }
}
